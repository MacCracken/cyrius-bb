...
**0.8.0** — audio backend moves to ALSA via vani; Cyrius toolchain `6.2.2 → 6.3.5`. The square-wave SFX synthesis (`src/audio.cyr`) is unchanged, but playback (`src/sound.cyr`) now routes through vani's `audio_*` core shim (vendored `vendor/vani-core.cyr`, vani 0.9.6 `core` profile) — direct ALSA PCM ioctls in pure Cyrius — replacing the legacy OSS `/dev/dsp` sink that was silent on modern ALSA-only systems. cyrius-bb is now a downstream **vani** consumer. The 6.3.5 bump clears the pin-drift warning and the `ERR_INVALID_INPUT` duplicate-symbol collision (6.3.5's sigil namespaces its error enum to `SIGIL_ERR_*`, leaving sankoch the sole definer); it also required `random` in `[deps].stdlib` (6.3.x sigil calls `random_bytes`) and explicit `lib/thread_local.cyr` + `lib/bench.cyr` includes (both became manual-include at 6.3.x). 222 assertions green; lint clean. (Prior: 0.7.2 — game-loop review; 0.7.1 — console-input fix; 0.7.0 — M6 polish; 0.6.0 — M5 scores; 0.5.0 — M4 audio; 0.4.0 — M3 depth; 0.3.0 — M2 levels; 0.2.0 — M1 loop; 0.1.0 — scaffold.)

- **DCE binary**: **1,366,808 B** (x86_64, static, stripped) — up from 1,114,888 B at 0.7.2: the `6.2.2 → 6.3.5` stdlib bundles grew, `random` was added, and sigil's crypto-TLS path is now compiled (it pulls `thread_local`, previously a silent warning-stub). Plus ~30 KB for the vendored vani-core ALSA shim. ~+533 KB is 6.2.2's larger folded stdlib bundles (sankoch/sigil/keccak/ct); ~+120 KB is the `bayan` bundle (carved-out `bigint`/`u128`), kept for a warning-free build — its `u256_*` refs back sigil's otherwise-unreachable signature path. One residual advisory: `large static data (335 KB)` from the stdlib bundles' lookup tables (not game code — the framebuffer is heap-allocated). The save-deps cost remains the bulk ([note 002](../architecture/002-save-deps-binary-size.md)); M1–M4 were ~100 KB bare-stdlib.
...
- **Deps**: stdlib + sankoch + sigil (+ transitive freelist/**bayan**/ct/keccak/thread_local/**random**), all from the 6.3.5 toolchain snapshot — no git fetch. `random` added (6.3.x sigil's HMAC/nonce path calls `random_bytes`); `thread_local` + the `bench` harness are manual-include modules at 6.3.x (included explicitly in `src/main.cyr` and the test/bench/fuzz entry points). Plus the vendored **vani-core** ALSA shim (`vendor/vani-core.cyr`, a committed single-file snapshot, not a git dep). shravan stays deferred (no consumable bundle).
- **Caveat**: the whole interactive flow (menu → play → pause → game-over → high-score entry) + `/dev/fb0` present (now resolution-probing) + audible ALSA (`/dev/snd`, via vani-core) + the *feel* of depth/audio/speed are build/lint + headless-smoke + frame/WAV-dump-verified only (no console/framebuffer/audio device in dev/CI). Headless logic is gated by 222 unit tests + the `<frames>` smoke; the save engine by an out-of-band disk round-trip. **First real-VT run (0.7.1) confirmed the framebuffer present works and fixed the blocking-input bug; a full end-to-end console playthrough remains the open pre-v1.0 gate.** Known remaining console gap: no `KDSETMODE`/`KD_GRAPHICS` VT switch, so `fbcon` text + cursor can fight the blit.

## Toolchain

- **Cyrius pin**: `6.3.5` (in `cyrius.cyml [package].cyrius`) — bumped from `6.2.2` 2026-06-29. Clears the pin-drift warning and the `ERR_INVALID_INPUT` collision (6.3.5's sigil namespaces to `SIGIL_ERR_*`). The bump needed three manifest/source edits beyond the pin number: (1) `random` added to `[deps].stdlib` (6.3.x sigil calls `random_bytes`); (2) `thread_local` is no longer auto-prepended even when listed — `src/main.cyr` + the test/bench/fuzz entry points now `include "lib/thread_local.cyr"` explicitly (6.3.2 makes a reachable undefined-fn call fatal, and sigil's auto-prepended crypto-TLS path calls `thread_local_*`); (3) the `bench` harness moved to `lib/bench.cyr` (`bench_new`/`bench_run`/`bench_report`) — the old runtime-injected `bench()` is gone. 222 assertions green, lint clean. (History: `6.0.1 → 6.2.2` 2026-06-13; `5.7.11 → 6.0.1` 2026-05-25; the 6.1.x "bayan distfile carve" replaced `bigint` with the **bayan** bundle.)
...
- `src/sound.cyr` — best-effort ALSA SFX sink via vani-core (`audio_open_playback`/`set_params`/`prepare` → `audio_write_bytes` → `drain`/`close`, to `/dev/snd/pcmC{card}D{device}p`; replaced the OSS `/dev/dsp` sink at 0.8.0; untested in CI; device only)
